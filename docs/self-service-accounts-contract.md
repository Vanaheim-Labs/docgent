...
1. Authenticate identity/credential, resolve target workspace from authoritative server mapping, check live account/membership/credential state, then operation entitlement, then document editorial policy/CAS. Unknown state, unreadable authority or mixed bindings deny access; never fall back from commercial to legacy.
...
4. Preserve explicit-header precedence: a bad/revoked token plus valid cookie remains denied. Agents cannot complete human review/release, create/revoke peers, manage membership, change bindings or buy a plan. Owner-only credential management; CSRF/origin protection for cookie-authenticated mutations.
5. Human exact-SHA status policy and stale-write handling remain mandatory. Entitlement allowance never overrides editorial locks; an approved document is not editable just because the trial is active.
...
| `internal` entitlement | Explicit operator mapping only | Existing internal operation policy without commercial timers; still enforce workspace ownership and editorial checks. |
...
| Docs/doc/diff/restore/rewrite accept/status APIs | Workspace binding before store reads; exact SHA/editorial rules unchanged; no hosted rewrite for commercial workspaces. Quota reservation only for new docs, not each edit. |
| Library/document/edit pages | Fetch only authorized workspace stores; no data read before scope check. UI reasons for expiry/locks; server still enforces. |
...
- Agent credentials must be workspace-scoped, independently revocable and never confer human review. OAuth email allowlist is internal compatibility, not customer ownership.
...
Additional characterization tables exercise internal agents with commercial brand/document usage above caps and exhausted render availability, long after commercial expiry. Internal allowances carry no commercial reservation labels, while credential revocation/expiry, membership revocation, account/workspace status, operation/profile/workspace/actor scope and all six human-only operations still deny as appropriate. These existing behaviors required no production changes. No live authentication, revocation store or concurrent mutation was exercised. The final-profile atomic reservation requirement in §3 remains a future adapter obligation.
