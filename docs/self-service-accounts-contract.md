...
Additional characterization tables exercise internal agents with commercial brand/document usage above caps and exhausted render availability, long after commercial expiry. Internal allowances carry no commercial reservation labels, while credential revocation/expiry, membership revocation, account/workspace status, operation/profile/workspace/actor scope and all six then-human-only operations still denied as appropriate. Those characterization tests required no production changes at that time. The later policy alignment in `6ddddaf` removed review/status from that deny list; the four account/credential/billing operations remain human-only. No live authentication, revocation store or concurrent mutation was exercised. The final-profile atomic reservation requirement in §3 remains a future adapter obligation.
...
## 12. Post-merge contract alignment

PR #34 was merged as `f75bb4c16ce0881e2a825802bd1cabce0dfda797` with the scoped-agent policy change from `6ddddaf`. PRs #35 and #36 had already removed editorial locks and human-only review/status gates from the runtime and UI. This follow-up aligns the contract with those changes; it does not add runtime policy callers or alter account/credential/billing restrictions, ownership, entitlements or exact-SHA/CAS requirements.

Four additional characterization tests exercise `review.complete` and `status.change` for commercial and internal agents. They verify explicit scope is necessary, unrelated or sibling operation scope does not suffice, live-authority failures deny, cross-workspace access denies, and commercial trial expiry still blocks these writes. They test existing behavior; no new production implementation or RED-to-GREEN behavior change is claimed.

Verification for this follow-up: `node --test apps/studio/test/workspace-policy.test.mjs` passed **90 tests**; `npm test` passed **172 tests**, with zero failed/skipped/todo. `npm exec --workspace apps/studio -- next build` exited 0 with compilation, lint/type checks, **7/7 static pages** and build traces complete. This direct command avoids the private-brands prebuild. The historical results in §11 remain evidence for the earlier slice, not current totals.

Historical §11 logs: the review RED log is `/tmp/docgent-a1-transition-red.log`; final test/build logs are `/tmp/docgent-a1-transition-policy.log`, `/tmp/docgent-a1-transition-full.log` and `/tmp/docgent-a1-transition-build.log` (local only, not repository artifacts or durable CI evidence). These tests and build establish local pure-policy behavior only. No OAuth, public signup, persistent registry, live revocation, worker enforcement, storage isolation, billing, durable output delivery, concurrent final-profile preservation or concurrent quota enforcement has been integrated or exercised. A2 resource gates and the later launch gates are unchanged.
